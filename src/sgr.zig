...
pub inline fn format(comptime text: []const u8, comptime opening_modifiers: []const SGRModifier, comptime closing_modifiers: []const SGRModifier) []const u8 {
    comptime {
        var open: []const u8 = &.{};
        var close: []const u8 = &.{};
...
pub const Parser = struct {
    ...
    pub inline fn parse(comptime text: []const u8) []const u8 {
        comptime {
    ...
    fn hexToRgb(hex: []const u8) []const u8 {
        ...
        const color = fmt.parseInt(i32, hex, 16) catch @compileError("Failed to parse color");
